...
VOID CExpr::ScanBase(CONST std::vector<std::wstring>&)
{
	...
	dwBase = ScanBinary.FindBase("C6??????000000FF??????????8B??E8", 0xBB4158 - 0xBB415F, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define UI偏移2 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("FF????8B??????00003B??????00000F??????????80", 0xBB2916 - 0xBB2919, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define UI遍历偏移1 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("8B??80??????0000000F??????????80??????00000074", 0xBB4CC0 - 0xBB4CC2, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define Ui显示偏移 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("84??0F??????????8B????????00FF????????00E8", 0x7E54AF - 0x7E54B7, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define 人物UI偏移 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("84??0F??????????8B????????00FF????????00E8", 0x7E54AF - 0x7E54F5, 2, 0x0, L"PathOfExile.exe"); 
	LOG_C_D(L"#define 背包UI偏移 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("752D80??????00000174", 0x788E5E - 0x788E51, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define 异界地图宝鉴偏移 0x%X", dwBase);

	dwBase = ScanBinary.FindBase("E8????????8B????8B??83??03", 0x788A0D - 0x7889BD, 2, 0x0, L"PathOfExile.exe");
	LOG_C_D(L"#define Npc出售窗口偏移 0x%X", dwBase);
